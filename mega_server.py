...
class MegaClient:
    ...
    def get_download_url(self, file_info: dict) -> str:
        """يحصل على رابط تحميل مؤقت من MEGA"""
        res = self._r({'a':'g','g':1,'n':file_info['h']})
        if isinstance(res, int): raise Exception(f"get_url: {res}")
        return res.get('g','')
...
def h_files(qs, b):
    """قائمة كل الملفات في MEGA (غير محادثات)"""
    try:
        files = get_memory().mega.files()
        result = []
        for f in files:
            if not (f['name'].startswith('chat_') and f['name'].endswith('.json.gz')):
                result.append({
                    'name': f['name'],
...
def h_download(qs, b):
    """تحميل ملف من MEGA — يعيد رابط مباشر"""
    name = _qget(qs, 'name')
    if not name: return 400, {'error': 'name missing'}
    try:
        f = get_memory().mega.find(name)
        if not f: return 404, {'error': 'file not found'}
        url = get_memory().mega.get_download_url(f)
        return 200, {'url': url, 'name': name, 'size_kb': round(f['size']/1024,1)}
    except Exception as e: return 500, {'error': str(e)}

def h_chat(qs, b):
    """✨ AI chat عبر Anthropic API — بديل Puter عند الحاجة"""
    if not ANTHROPIC_KEY:
        return 503, {'error': 'ANTHROPIC_API_KEY غير مُعيَّن — عيّنه بـ export ANTHROPIC_API_KEY=sk-ant-...'}
    messages = b.get('messages', [])
    model    = b.get('model', 'claude-sonnet-4-6')
    if not messages: return 400, {'error': 'messages missing'}
    try:
        r = req_lib.post(
        ...
        if not r.ok: return r.status_code, {'error': r.text[:300]}
        data = r.json()
        text = data.get('content', [{}])[0].get('text', '')
        return 200, {'content': text, 'model': model}
    except Exception as e: return 500, {'error': str(e)}
...
if USE_FLASK:
    ...
    def r5():
        ...
        return jsonify(d),c
    @app.route('/files')
    def r_files(): c,d=h_files({},{}); return jsonify(d),c
    @app.route('/download')
    def r_dl():
        c,d=h_download({'name':[freq.args.get('name','')]},{})
        return jsonify(d),c
    @app.route('/chat', methods=['POST'])
    def r_chat(): c,d=h_chat({},freq.get_json() or {}); return jsonify(d),c
    @app.route('/stats')
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            r = {'/health':h_health,'/list':h_list,'/load':h_load,
                 '/context':h_context,'/stats':h_stats,'/files':h_files,'/download':h_download}.get(prs.path)
        ...
        def do_POST(self):
            ...
            r = {'/save':h_save,'/delete':h_delete,'/chat':h_chat}.get(prs.path)
